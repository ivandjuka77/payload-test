...
export function ServiceHero({ service }: any) {
  const {
    imageSrc,
    imageAlt,
    serviceName,
    ...
    <section className="relative w-full min-h-[600px] flex items-center">
      ...
      <div className="container relative z-10 px-4 md:px-6 mx-auto py-20">
        <div className="max-w-3xl">
          ...
          <p className="text-xl text-white/90 font-secondary mb-8 max-w-2xl">{description}</p>

          {accreditations && accreditations.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-8">
              {accreditations.map((accreditation: any, index: any) => (
          ...
          {headOfDepartment && (
            <div className="inline-block bg-primary-dark/60 px-4 py-2 rounded-md mb-8">
              ...
              <p className="text-white font-semibold">{headOfDepartment}</p>
            </div>
          )}
